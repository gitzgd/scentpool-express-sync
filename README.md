...
总部发货后台顶部的“异常提醒”角标汇总面单下单、物流查询、退货物流查询和打印失败，以及等待超过 30 分钟的面单任务。点击角标后按电子面单、物流查询和打印分类查看；每项包含业务编号、失败原因和操作建议，失败批次只会重新提交失败订单。提醒列表每 60 秒自动刷新，底层状态恢复后会自动消失；线下已经处理的异常可标记“人工已处理”，只隐藏当前这一次，后续再次失败仍会重新提醒。页面无法读取异常状态时会显示红色提示，恢复前不要重复提交同一批任务。
