...
发货后台和门店看板由服务器按每页 50 单读取；切页不重复加载商品、门店、面单设置和异常提醒。发货后台的“批量下单”仍读取当前筛选条件下的全部可下单订单，不受页面 50 单限制；“批量打印面单”则明确只选择当前页，便于在 50 单安全上限内逐页打印。系统按持久化队列逐单取号并生成面单；成功后保存快递单号、任务 ID、面单地址和打印状态，订单改为“已发货”，随后立即查询一次物流。快递100返回 `30011` 且包含完整运单号和任务号时按“重新取得面单成功”处理，不再误报失败。限流、服务端错误、超时和临时网络错误会使用原请求编号自动重试，避免重复取号；中断超过 10 分钟的任务会自动恢复，连续 3 次仍未完成则停止自动尝试并明确标为失败。
