...
| 发货单 | 门店创建、看板、筛选、编辑限制、总部处理 | `database.py`, `server.py`, `static/app.js` |
...
| [脱敏业务日报](daily-audit.md) | 独立 Bearer 鉴权、只读 SQLite 汇总、历史口径限制（已开发、未部署/待验收） | `database.py`, `server.py`, `daily_audit_test.py` |
