...
- 状态：基础版生产已启用（2026-08-05）；历史日末与脱敏失败事件扩展已开发，待审查/部署（2026-08-15）
...
基础版在 2026-08-05 不修改 Schema。2026-08-15 的扩展尚未部署；部署会增量创建 `audit_event_meta`、`audit_state_events`、索引和触发器，并为现有记录写入一次脱敏状态快照，不修改业务表状态或个人字段。
