...
- 状态：基础版生产已启用（2026-08-05）；历史日末与脱敏失败事件扩展已上线（2026-08-17）；连接/打印/延迟兼容及时间证据等级已上线（2026-08-29）
...
2026-08-29 的连接/打印/延迟兼容扩展不需要新增环境变量。部署后已用固定采集器完成真实 30 秒双采样；健康、日报、连接守恒和活动连接回落均通过。Render 延迟接口仍返回通道级 HTTP 错误，因此总状态保持 `error`，不能据此宣称网站不可用或延迟正常。本机副本只能通过 `scripts/install_daily_audit_probe.py` 原子安装；macOS 使用系统 CA 文件校验 HTTPS，避免 Python 自带证书链缺失造成全通道误报。回滚代码即可移除最小诊断路由和固定打印事件；本机采集器应同样用安装器回退到已审查的仓库版本。
